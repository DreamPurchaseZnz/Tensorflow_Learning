# Author: https://timsainburg.com/python-mel-compression-inversion.html
# Place: Shenzhen
# Time: 2019-5-8
# Copyright
...
# Most of the Spectrograms and Inversion are taken from:
...
print(isinstance(data.dtype, np.float32))
...
scaled = np.int16(recovered_audio_orig/np.max(np.abs(recovered_audio_orig)) * 32767)
wavfile.write("recovered_audio_orig.wav", rate, data=scaled)   # play the audio
...
wavfile.write("inverted_mel_audio.wav", rate, data=scaled)   # play the audio
